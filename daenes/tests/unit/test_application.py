...
def make_application(clock, outcomes: list) -> tuple[Application, MagicMock]:
...
def test_a_successful_run_waits_out_the_success_interval(clock):
    application, _ = make_application(clock, [None, EndOfTest()])
...
def test_a_retryable_error_waits_out_the_retry_interval(clock):
    ...
    application, _ = make_application(clock, [RetryableError("down"), EndOfTest()])
...
def test_an_unretryable_error_is_left_to_the_caller(clock):
    ...
    application, synchronizer = make_application(clock, [ValueError("unretryable")])
...
def test_run_survives_the_docker_daemon_going_away(clock):
    ...
    application, synchronizer = make_application(clock, outcomes)
    ...
    assert clock.slept == [RETRY_INTERVAL, RETRY_INTERVAL, SUCCESS_INTERVAL]
