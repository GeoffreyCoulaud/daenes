...
# (along with the `daenes` console script) and no source is needed at runtime.
COPY pyproject.toml uv.lock README.md ./
COPY daenes ./daenes

# No uv cache mount here: with a constant name+version it would serve a stale
# `daenes` wheel when only source files change. Building fresh is cheap (deps
# are already in the venv) and guarantees the package matches the source.
...
# Only the venv: it carries the installed package and the `daenes` entry point.
COPY --from=builder /app/.venv /app/.venv

# Where the zone files are written, meant to be mounted over. The docker socket
# daenes reads the deployment through is mounted at /var/run/docker.sock.
VOLUME /zones

ENV PATH="/app/.venv/bin:$PATH"
ENV PYTHONUNBUFFERED=1
ENTRYPOINT [ "/app/.venv/bin/daenes" ]
